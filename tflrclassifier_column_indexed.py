...
"""
File: tflrclassifier_column_indexed.py
...
class TFLRClassifier(object):
    ...
    """
    def __init__(self, feature_num, l2_weight=0.01, batch_size=-1,
    ...
    def _build_graph(self):
        ...
        with self.graph.as_default():
            ...
            self.loss = tf.reduce_mean(
                ...
                + self.l2_weight * tf.nn.l2_loss(self.embeddings) \
                + self.l2_weight * tf.nn.l2_loss(self.bias)
    ...
    def _construct_weights(self):
        """
            Construct weights
        """
        self.embeddings = tf.get_variable(name='embeddings', shape=[self.feature_num, 1],
    ...
    def _forward_pass(self):
        ...
        h = tf.multiply(tf.nn.embedding_lookup(self.embeddings, self.X_colind),
            tf.reshape(self.X_colval, shape=[-1, self.feature_num, 1])) # shaped [-1, feature_num, factor_num]
        h = tf.reduce_sum(h, axis=2) # shaped [-1, feature_num]
        h = tf.reduce_sum(h, axis=1, keepdims=True) # shaped [-1]
